locals {
  ...
  list_origin_identity          = local.create_origin_access_identity ? [for v in aws_cloudfront_origin_access_identity.cdf_origin_access_identity : v.id] : []
}

resource "aws_cloudfront_origin_access_identity" "cdf_origin_access_identity" {
...
resource "aws_cloudfront_distribution" "cdf_distribution" {
  ...
  dynamic "origin" {
    ...
    content {
      domain_name         = origin.value.domain_name[0]
      ...
      dynamic "s3_origin_config" {
        ...
        content {
          origin_access_identity = lookup(s3_origin_config.value, "cloudfront_access_identity_path", lookup(lookup(aws_cloudfront_origin_access_identity.cdf_origin_access_identity, lookup(s3_origin_config.value, "origin_access_identity", ""), {}), "cloudfront_access_identity_path", null))
        }
      }
...
resource "aws_cloudfront_monitoring_subscription" "cdf_monitoring_subscription" {
  count = var.create_distribution && var.create_monitoring_subscription ? 1 : 0

  distribution_id = aws_cloudfront_distribution.cdf_distribution[0].id
...
resource "aws_s3_bucket_policy" "content-manager-web" {
  bucket = var.bucket_s3_id[0]
  ...
  "Statement": [
    {
        ...
        "Effect": "Allow",
        "Principal": {
            "AWS": "arn:aws:iam::cloudfront:user/CloudFront Origin Access Identity ${local.list_origin_identity[0]}"
        },
        "Action": "s3:GetObject", 
        "Resource": "${var.bucket_s3_arn[0]}/*"
    }
  ]
}
EOT
